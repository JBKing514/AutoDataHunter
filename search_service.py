...
def _extract_source_urls(tags: list[str]) -> tuple[str, str]:
    ...
    for tag in tags or []:
        ...
        if not v.startswith("http://") and not v.startswith("https://"):
            v = f"https://{v}"
        if "external_backup_url" in v:
            external_backup_url = external_backup_url or v
        elif "external_url" in v:
            external_url = external_url or v
    return external_url, external_backup_url


def _prefer_ex(cfg: dict[str, Any]) -> bool:
    base = str(cfg.get("external_BASE_URL") or "").strip().lower()
    cookie = str(cfg.get("external_COOKIE") or "").strip()
    return ("external_backup_url" in base) and bool(cookie)


def _prefer_link(external_url: str, external_backup_url: str, cfg: dict[str, Any]) -> str:
    external_u = str(external_url or "").strip()
    ex_u = str(external_backup_url or "").strip()
    if _prefer_ex(cfg):
        if not ex_u and external_u and "external_url" in external_u:
            ex_u = external_u.replace("https://external_url/", "https://external_backup_url/")
        return ex_u or external_u
    return external_u or ex_u
...
def _item_from_work(row: dict[str, Any], cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    external_url, external_backup_url = _extract_source_urls(tags)
    ...
    return {
        ...
        "link_url": _prefer_link(external_url, external_backup_url, cfg_use),
        "category": category,
        ...
        "meta": {
            ...
            "external_posted": _norm_epoch(row.get("external_posted")),
...
def _item_from_external(row: dict[str, Any], cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    cfg_use = cfg or resolve_config()[0]
    gid = int(row.get("gid") or 0)
    token = str(row.get("token") or "")
    external_url = str(row.get("external_url") or "")
    external_backup_url = str(row.get("external_backup_url") or "")
    category = _category_from_tags([str(x) for x in (row.get("tags") or [])], str(row.get("category") or ""))
    return {
        "id": f"external:{str(gid or '')}:{token}",
        "source": "external_works",
        "gid": gid,
        "token": token,
        ...
        "link_url": _prefer_link(external_url, external_backup_url, cfg_use),
        "category": category,
        "thumb_url": f"/api/thumb/external/{gid}/{quote(token, safe='')}",
        "reader_url": "",
        "score": float(row.get("score") or 0.0),
        "meta": {
...
def _tag_candidates(ttl_s: int = 900) -> list[str]:
    ...
    rows = query_rows(
        ...
        "UNION ALL "
        "SELECT unnest(tags) AS tag FROM external_works "
        "UNION ALL "
        "SELECT unnest(tags_translated) AS tag FROM external_works"
...
def _hot_tags(limit: int = 1500, min_freq: int = 5) -> list[str]:
    rows = query_rows(
        "SELECT tag FROM ("
        "SELECT unnest(tags) AS tag FROM works "
        "UNION ALL SELECT unnest(tags) AS tag FROM external_works "
        "UNION ALL SELECT unnest(tags_translated) AS tag FROM external_works"
...
def _search_text_non_llm(
    ...
    if scope in ("works", "both"):
        rows = query_rows(
            "SELECT arcid, title, tags, external_posted, date_added, lastreadtime "
            "FROM works "
    ...
    if scope in ("external", "both"):
        rows = query_rows(
            "SELECT gid, token, external_url, external_backup_url, title, title_jpn, category, tags, tags_translated, posted, filecount "
            "FROM external_works "
        ...
        for r in rows:
            ...
            items.append(_item_from_external({**r, "score": score}, cfg))
...
def _search_by_visual_vector(
    ...
    if scope in ("works", "both"):
        ...
        for r in works_rows:
            ...
            items.append(_item_from_work({**r, "score": score}, cfg))
    if scope in ("external", "both"):
        external_rows = query_rows(
            "SELECT gid, token, external_url, external_backup_url, title, title_jpn, category, tags, tags_translated, posted, filecount, "
            "(cover_embedding <=> (%s)::vector) AS dist "
            "FROM external_works WHERE cover_embedding IS NOT NULL "
...
def _scenario_weights(cfg: dict[str, Any], scenario: str) -> dict[str, float]:
    ...
    if sc == "visual":
        return {
            ...
            "external_visual": float(cfg.get("SEARCH_WEIGHT_external_VISUAL", 1.6) or 1.6),
            "desc": float(cfg.get("SEARCH_WEIGHT_DESC", 0.8) or 0.8),
            "text": float(cfg.get("SEARCH_WEIGHT_TEXT", 0.7) or 0.7),
            "external_text": float(cfg.get("SEARCH_WEIGHT_external_TEXT", 0.7) or 0.7),
        }
    if sc == "mixed":
        return {
            ...
            "external_visual": float(cfg.get("SEARCH_WEIGHT_MIXED_external_VISUAL", 1.0) or 1.0),
            "desc": float(cfg.get("SEARCH_WEIGHT_MIXED_DESC", 1.4) or 1.4),
            "text": float(cfg.get("SEARCH_WEIGHT_MIXED_TEXT", 0.9) or 0.9),
            "external_text": float(cfg.get("SEARCH_WEIGHT_MIXED_external_TEXT", 0.9) or 0.9),
        }
    return {
        ...
        "external_visual": float(cfg.get("SEARCH_WEIGHT_PLOT_external_VISUAL", 0.5) or 0.5),
        "desc": float(cfg.get("SEARCH_WEIGHT_PLOT_DESC", 2.0) or 2.0),
        "text": float(cfg.get("SEARCH_WEIGHT_PLOT_TEXT", 0.9) or 0.9),
        "external_text": float(cfg.get("SEARCH_WEIGHT_PLOT_external_TEXT", 0.9) or 0.9),
    }
...
def _agent_nl_search(
    ...
    *,
    ...
    channels: dict[str, list[str]] = {"text": [], "external_text": [], "desc": [], "visual": [], "page_visual": [], "external_visual": []}
    try:
        if scope in ("works", "both"):
            rows = query_rows(
        ...
        if scope in ("external", "both"):
            rows = query_rows(
                "SELECT gid, token FROM external_works "
                "WHERE (title ILIKE %s OR title_jpn ILIKE %s OR array_to_string(tags, ' ') ILIKE %s OR array_to_string(tags_translated, ' ') ILIKE %s) "
                "OR (tags && %s::text[]) OR (tags_translated && %s::text[]) "
                "ORDER BY posted DESC NULLS LAST LIMIT %s",
                (f"%{q}%", f"%{q}%", f"%{q}%", f"%{q}%", filter_tags or [""], filter_tags or [""], int(n)),
            )
            channels["external_text"] = [f"external:{int(r.get('gid') or 0)}:{str(r.get('token') or '').strip()}" for r in rows if int(r.get("gid") or 0) > 0 and str(r.get("token") or "").strip()]
    except Exception as e:
    ...
    try:
        ...
        if scope in ("external", "both"):
            rows = query_rows(
                "SELECT gid, token FROM external_works WHERE cover_embedding IS NOT NULL "
                "ORDER BY cover_embedding <=> (%s)::vector LIMIT %s",
                (vtxt2, int(n)),
            )
            channels["external_visual"] = [f"external:{int(r.get('gid') or 0)}:{str(r.get('token') or '').strip()}" for r in rows if int(r.get("gid") or 0) > 0 and str(r.get("token") or "").strip()]
    except Exception as e:
    ...
    external_parts = [x.split(":", 2) for x in ranked_ids if x.startswith("external:")]
    external_pairs = [(int(p[1]), p[2]) for p in external_parts if len(p) == 3 and p[1].isdigit() and p[2]]
    work_rows = query_rows(
        "SELECT arcid, title, tags, external_posted, date_added, lastreadtime FROM works WHERE arcid = ANY(%s)",
        (work_ids or [""],),
    ) if work_ids else []
    external_rows = query_rows(
        "SELECT gid, token, external_url, external_backup_url, title, title_jpn, category, tags, tags_translated, posted, filecount "
        "FROM external_works WHERE (gid, token) IN (SELECT * FROM unnest(%s::int[], %s::text[]))",
        ([x[0] for x in external_pairs] or [0], [x[1] for x in external_pairs] or ["_"]),
    ) if external_pairs else []
    wm = {f"work:{str(r.get('arcid') or '').strip()}": _item_from_work(r, cfg) for r in work_rows}
    em = {f"external:{int(r.get('gid') or 0)}:{str(r.get('token') or '').strip()}": _item_from_external(r, cfg) for r in external_rows}
...
def _uploaded_image_search(
    ...
    *,
    ...
    if scope_use not in ("works", "external", "both"):
        scope_use = "both"
