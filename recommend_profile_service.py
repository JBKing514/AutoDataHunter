...
_KEY_RE = re.compile(r"^external:(\d+):([a-zA-Z0-9]+)$")
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            ...
            for gid, tok in gid_tok:
                cur.execute(
                    "SELECT cover_embedding::text AS vec FROM external_works WHERE gid = %s AND token = %s LIMIT 1",
                    (int(gid), str(tok)),
                )
                r = cur.fetchone() or {}
                v = _project_1024(_parse_vector_text(str(r.get("vec") or "")))
                if len(v) == 1024:
                    vec_map[f"external:{gid}:{tok.lower()}"] = _normalize_l2(v)
