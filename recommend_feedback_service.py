...
def parse_gallery_identity(gid: int | None = None, token: str = "", external_url: str = "", external_backup_url: str = "") -> tuple[str, str]:
    gid_s = str(gid or "").strip()
    tok = str(token or "").strip()
    if gid_s and tok:
        return gid_s, tok
    for url in (external_url, external_backup_url):
...
def record_recommend_click(*, user_id: str, gid: int | None, token: str, external_url: str = "", external_backup_url: str = "", weight: float = 1.0) -> dict[str, Any]:
    gid_s, tok = parse_gallery_identity(gid=gid, token=token, external_url=external_url, external_backup_url=external_backup_url)
...
def record_recommend_impressions(*, user_id: str, items: list[dict[str, Any]], weight: float = 1.0) -> dict[str, Any]:
    ...
    for it in items or []:
        gid_s, tok = parse_gallery_identity(
            ...
            external_url=str(it.get("external_url") or ""),
            external_backup_url=str(it.get("external_backup_url") or ""),
        )
        key = recommendation_key(gid_s, tok)
...
def record_recommend_dislike(*, user_id: str, gid: int | None, token: str, external_url: str = "", external_backup_url: str = "", weight: float = 1.0) -> dict[str, Any]:
    gid_s, tok = parse_gallery_identity(gid=gid, token=token, external_url=external_url, external_backup_url=external_backup_url)
    key = recommendation_key(gid_s, tok)
    if not key:
        return {"ok": False, "recorded": False, "reason": "invalid gallery identity"}
